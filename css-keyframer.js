...
(function (global, factory) {
	...
	var isPlainObject = (index$2 && typeof index$2 === 'object' && 'default' in index$2 ? index$2['default'] : index$2);
	...
	function indent(str, size) {
	  ...
	  return str.split(eol).map(function (line) {
	    return new Array(size + 1).join(" ") + line;
	  }).join(eol);
	}
	...
	function makeKeyframes(name, props) {
	  ...
	  var prefix = getKeyframesPrefix();
	  var styles = [];

	  each(props, function (values, selector) {
	    var styleString = makeStyle(selector, values);
	    styles.push(styleString);
	  });

	  return ["@" + prefix + "keyframes " + name + " {", indent(styles.join("\n\n"), 2), "}"].join("\n");
	}
	...
	var ATTR_NAME = "data-keyframe-name";

	function getStyleElement(name) {
	  if (IS_SERVER_SIDE) return null;

	  var el = document.querySelector("style[" + ATTR_NAME + "=\"" + name + "\"]");
	...
	var CssKeyframer = function () {
	  ...
	  createClass(CssKeyframer, [{
	    ...
	    value: function register(name, keyframe) {
	      ...
	      var el = getStyleElement(name);
	      var keyframeString = makeKeyframes(name, keyframe);
	...
	var cssKeyframer = new CssKeyframer();

	cssKeyframer.register("spin", {
	  "0%": {
